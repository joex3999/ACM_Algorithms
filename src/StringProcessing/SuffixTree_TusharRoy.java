 ...
 *
 *         Solution Rule 1: For phase i+1 if S[j..i] ends at last character of
 *         leaf edge then add S[i+1] at the end. Rule 2: For phase i+1 if
 *         S[j..i] ends somewhere in middle of edge and next character is not
 *         S[i+1] then a new leaf edge with label S[i+1] should be created Rule
 *         3: For phase i+1 if S[j..i] ends somewhere in middle of edge and next
 *         character is S[i+1] then do nothing(resulting in implicit tree)
 *
 *         Suffix Link: For every node with label x@ where x is a single
 *         character and @ is possibly empty substring there is another node
 *         with label x. This node is suffix link of first node. If @ is empty
 *         then suffix link is root.
 *
 *         Trick1 Skip/Count trick While traveling down if number of characters
 *         on edge is less than number of characters to traverse then skip
 *         directly to the end of the edge. If number of characters on label is
 *         more than number of characters to traverse then go directly to that
 *         character we care about.
 *
 *         Edge-label compression Instead of storing actual characters on the
 *         path store start and end indices on the path.
 *
 *         Trick2 - Stop process as soon as you hit rule 3. Rule 3 is show
 *         stopper
 *
 *         Trick3 - Keep a global end on leaf to do rule 1 extension.
 *
 *         Active point - It is the point from which traversal starts for next
 *         extension or next phase. Active point always starts from root. Other
 *         extension will get active point set up correctly by last extension.
 *
 *         Active node - Node from which active point will start Active Edge -
 *         It is used to choose the edge from active node. It has index of
 *         character. Active Length - How far to go on active edge.
 *
 *         Active point rules 1) If rule 3 extension is applied then active
 *         length will increment by 1 if active length is not greater then
 *         length of path on edge. 2) If rule 3 extension is applied and if
 *         active length gets greater than length path of edge then change
 *         active node, active edge and active length 3) If active length is 0
 *         then always start looking for the character from root. 4) If rule 2
 *         extension is applied and if active node is root then active edge is
 *         active edge + 1 and active length is active lenght -1 5) If rule 2
 *         extension is applied and if active node is not root then follow
 *         suffix link and make active node as suffix link and do no change
 *         anything.
 *
 *         Test cases adeacdade abcabxabcd abcdefabxybcdmnabcdex abcadak
 *         dedododeodo abcabxabcd mississippi banana ooooooooo
 *
 *         References http://web.stanford.edu/~mjkay/gusfield.pdf
 *         http://www.geeksforgeeks
 *         .org/ukkonens-suffix-tree-construction-part-6/
 *         https://www.cs.helsinki.fi/u/ukkonen/SuffixT1withFigs.pdf
 *         https://gist.github.com/axefrog/2373868
 */
public class SuffixTree_TusharRoy {
	public static void main(String args[]) {
		SuffixTree_TusharRoy st = new SuffixTree_TusharRoy("mississippi".toCharArray());
		st.build();
		st.dfsTraversal();
	...
	public SuffixTree_TusharRoy(char input[]) {
	...
	private void startPhase(int i) {
		...
		// global end for leaf. Does rule 1 extension as per trick 3 by
		// incrementing end.
		end.end++;
		...
		while (remainingSuffixCount > 0) {
			...
			if (active.activeLength == 0) {
				// if current character from root is not null then increase
				// active length by 1
				// and break out of while loop. This is rule 3 extension and
				...
				try {
					char ch = nextChar(i);
					// if next character is same as current character then do a
					// walk down. This is again a rule 3 extension and
					// trick 2 (show stopper).
					if (ch == input[i]) {
						// if lastCreatedInternalNode is not null means rule 2
						// extension happened before this. Point suffix link of
						// that node
						// to selected node using active point.
						// TODO - Could be wrong here. Do we only do this if
						// when walk down goes past a node or we do it every
						// time.
						if (lastCreatedInternalNode != null) {
							lastCreatedInternalNode.suffixLink = selectNode();
						}
						// walk down and update active node if required as per
						// rules of active node update for rule 3 extension.
						walkDown(i);
						break;
					} else {
						// next character is not same as current character so
						// create a new internal node as per
						// rule 2 extension.
						...
						SuffixNode newInternalNode = SuffixNode.createNode(
								oldStart, new End(oldStart
										+ active.activeLength - 1));
						// create new leaf node
						SuffixNode newLeafNode = SuffixNode.createNode(i,
								this.end);
						// set internal nodes child as old node and new leaf
						// node.
						...
						// if another internal node was created in last
						// extension of this phase then suffix link of that
						// node will be this node.
						if (lastCreatedInternalNode != null) {
							lastCreatedInternalNode.suffixLink = newInternalNode;
						}
						// set this guy as lastCreatedInternalNode and if new
						// internalNode is created in next extension of this
						// phase
						// then point suffix of this node to that node.
						// Meanwhile set suffix of this node to root.
						...
						// one and decrease active length by 1
						else {
					...
					// this happens when we are looking for new character from
					// end of current path edge. Here we already have internal
					// node so
					// we don't have to create new internal node. Just create a
					...
					// if active node is root then increase active index by one
	...
	private char nextChar(int i) throws EndOfPathException {
		SuffixNode node = selectNode();
		if (diff(node) >= active.activeLength) {
			return input[active.activeNode.child[input[active.activeEdge]].start
					+ active.activeLength];
		}
	...
	/**
	 * Do validation of the tree by comparing all suffixes and their index at
	 * leaf node.
	 */
	private boolean validate(SuffixNode root, char[] input, int index, int curr) {
		...
		if (root.index != -1) {
			if (root.index != index) {
				System.out.println("Index not same. Failed at " + curr
						+ " for index " + index);
				return false;
			} else {
				return true;
		...
		for (int i = node.start; i <= node.end.end; i++) {
			if (input[curr + j] != input[i]) {
				System.out.println("Mismatch found " + input[curr + j] + " "
						+ input[i]);
				return false;
			}
			j++;
